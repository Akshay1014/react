      ...
      <div className='py-10 h-screen'>
          ...
          <div>
              {queries.map((item, index) => (
                  <div key={index}>
                      <div onClick={() => handleClick(index)} className='text-2xl bg-gray-800 flex justify-between cursor-pointer hover:bg-gray-700 items-center p-5 px-10 mt-3'>
                          <div> {item.question} </div>
                          <div className="text-5xl">{activeIndex === index ? <FaTimes /> : <FaPlus />}</div>

                      </div>
                      {activeIndex===index &&(<div className='text-2xl p-5 bg-gray-800 mt-0.5'>
                          {item.answer}
                      </div>)}
                 </div>
             ))}
